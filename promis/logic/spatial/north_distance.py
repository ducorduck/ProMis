...
class NorthDistance:
    ...
    def from_range(self, low: float, high: float) -> RasterBand:
        probabilities = RasterBand(
            self.mean.data.shape, self.mean.origin, self.mean.width, self.mean.height
        )

        for x, y in product(range(self.mean.data.shape[0]), range(self.mean.data.shape[1])):
            cdf_high = Gaussian(self.mean.data[x, y].reshape((1, 1)), self.variance.data[x, y].reshape((1, 1))).cdf(array([high]))
            cdf_low = Gaussian(self.mean.data[x, y].reshape((1, 1)), self.variance.data[x, y].reshape((1, 1))).cdf(array([low]))
            probabilities.data[x, y] = cdf_high - cdf_low

        return probabilities

    def split(self) -> "list[list[NorthDistance]] | NorthDistance":
        ...
        return [
            [
                NorthDistance(mean_splits[0][0], variance_splits[0][0], self.location_type),
                NorthDistance(mean_splits[0][1], variance_splits[0][1], self.location_type),
            ],
            [
                NorthDistance(mean_splits[1][0], variance_splits[1][0], self.location_type),
                NorthDistance(mean_splits[1][1], variance_splits[1][1], self.location_type),
            ],
        ]
    ...
    def index_to_distributional_clause(self, index: tuple[int, int]) -> str:
        ...
        relation = f"distance_north(row_{index[1]}, column_{index[0]}, {feature_name})"
    ...
    def extract_parameters(location: CartesianLocation, str_trees: list[STRtree]) -> float:
        ...
        for str_tree in str_trees:
            ...
            distances_N.append(
                line.coords[0][1] - line.coords[1][1]          # location_y - des_y
            )

        return mean(distances_N), var(distances_N)
