...
class EastDistance:
    ...
    def split(self) -> "list[list[EastDistance]] | EastDistance":
        ...
        return [
            [
                EastDistance(mean_splits[0][0], variance_splits[0][0], self.location_type),
                EastDistance(mean_splits[0][1], variance_splits[0][1], self.location_type),
            ],
            [
                EastDistance(mean_splits[1][0], variance_splits[1][0], self.location_type),
                EastDistance(mean_splits[1][1], variance_splits[1][1], self.location_type),
            ],
        ]
    ...
    def index_to_distributional_clause(self, index: tuple[int, int]) -> str:
        ...
        relation = f"distance_east(row_{index[1]}, column_{index[0]}, {feature_name})"
    ...
    def extract_parameters(location: CartesianLocation, str_trees: list[STRtree]) -> float:
        ...
        for str_tree in str_trees:
            ...
            distances_E.append(
                line.coords[0][0] - line.coords[1][0]      #  location_x - des_x
            )

        return mean(distances_E), var(distances_E)
